...
What does it do?
It goes through a range and removes all non-unique elements that have returns a range of unique elements.
Of course the presupposition is that the range is sorted.
It will work if the range is not sorted either.
It will just not eliminate all equal elements.
It returns a pointer to the element passed the last 
element of the range. This is a standard STL convention.
