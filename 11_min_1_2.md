...
We will store this history as a list (using `list_pool`)
with each element in the binary counter.
Note that the counter works on generic elements, so it doesn't
need to be modified or know about this history tracking.
...
1. Do a `while` loop over a range of elements and add them to a `binary_counter`.
   ...
   We will store their iterators in our counter,
   rather than the values they point to, so we can return all the useful return (as discussed previously).
