...
void print_comparisons(size_t n) {
  ...
  for (size_t i(64); i < n; i <<= 1) {
    ...
    ::iota(buffer.begin(), buffer.end(), uint64_t(0)); // newer versions of std include iota
...
void print_times(size_t n) {
  ...
  for (size_t i(64); i < n; i <<= 1) {
    ...
    ::iota(buffer.begin(), buffer.end(), T(0)); // newer versions of std include iota
    std::srand ( unsigned (std::time( 0 ) ));
    std::random_shuffle(buffer.begin(), buffer.end());
...
int main() {
    ...
    print_times<uint64_t>(128 * 1024 * 1024);
}
